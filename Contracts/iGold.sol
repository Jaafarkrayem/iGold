...
contract iGold is ERC20, Ownable {
    ...
    struct VIP{
        ...
        uint256 usdtToGold;
        uint256 goldInGrams;
        uint256 goldInReserve;
        uint256 buyPeriodAllowance;
        uint256 iGoldAmount;
        uint256 usdtAmount;
        uint256 depositDate;
        uint256 secondRound;
    ...
    mapping(address => bool) public selfVIP;
    mapping(address => VIP) public lP;
    ...
    uint256 public externalGoldReserve; // in grams
    uint256 public usdtLiquidity;
    uint256 public iGoldLiquidity;
    ...
    uint256 public liquidityProviders;
    ...
    function setGoldBuyer(address _goldBuyer) external onlyOwner{
        require(_goldBuyer != address(0x0), "Zero address");
        goldBuyer = _goldBuyer;
    }
    ...
    function removeVIPMember(address _member) external onlyOwner {
        ...
        require(!selfVIP[_member], "iGold: not allowed remove of self VIP");

        vipMembers[_member] = false;
    }

    function becomeVIP(uint256 _amount) external{
        require(goldBuyer != address(0x0), "iGold: goldBuyer is zero address");
        uint256 _ouncePrice = uint256(getLatestGoldPriceOunce());
        require(_amount.mul(1e2) >= _ouncePrice, "1 Ounce in USDT at least");
        require(usdtToken.balanceOf(msg.sender) >= _amount, "iGold: Balance");
        uint256 _fees = _amount.div(66); // 1.515%
        uint256 afterFees = _amount.sub(_fees);
        require(usdtToken.transferFrom(msg.sender, goldBuyer, afterFees), "iGold: Transfer faild due to allowance");
        require(usdtToken.transferFrom(msg.sender, address(this), _fees), "//");
        lP[msg.sender].usdtToGold += afterFees;
        uint256 _goldInGrams = afterFees * (1e2) / uint256(getLatestGoldPriceGram());
        lP[msg.sender].goldInGrams += _goldInGrams;
        lP[msg.sender].secondRound = block.timestamp + 365 days;
        externalGoldReserve += _goldInGrams;
        //goldReserve += _goldInGrams;
        usdtVault += _fees;
        if(!vipMembers[msg.sender]){
            vipMembers[msg.sender] = true;
            selfVIP[msg.sender] = true;
        }
    ...
    function buy(uint256 _usdtAmount) external notPausedTrade returns (uint256){

    }

    function _buy(uint256 _usdtAmount) internal notPausedTrade returns (uint256){
    ...
    function addLiquidity(uint256 _usdtAmount)
        external
        onlyVIP
        notPausedTrade
    {
        uint256 gramPrice = uint256(getLatestGoldPriceGram());
        uint256 _iGoldAmount = _usdtAmount.mul(1e2).mul(1e1).mul(1e8).div(
            uint256(gramPrice)
        ); // 0.1g per token
        uint256 fromUserReservedGold = (_usdtAmount * 1e2) / gramPrice;
        goldReserve = goldReserve.add(fromUserReservedGold);

        require(usdtToken.transferFrom(msg.sender, address(this), _usdtAmount),"iGold: Transfer failed due to allowance");
        _mint(address(this), _iGoldAmount);

        iGoldLiquidity += _iGoldAmount;
        usdtLiquidity += _usdtAmount;
        
        lP[msg.sender].goldInGrams -= fromUserReservedGold;
        lP[msg.sender].goldInReserve += fromUserReservedGold;
        lP[msg.sender].iGoldAmount += _iGoldAmount;
        lP[msg.sender].usdtAmount += _usdtAmount;
        lP[msg.sender].depositDate = block.timestamp;
        ...
        emit goldReserved("Add", fromUserReservedGold, goldReserve);
    }
